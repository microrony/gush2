import { handleize, overallRating, getRatingValue } from './helpers.js';
...
const filterReviews = event => {
  ...
  let selectedFilter = handleize(event.target.getAttribute('data-filter'));
  ...
  filtered_reviews = allReviews.filter(review => {
    if (selectedFilter == 'all-reviews') {
      ...
      return (
        review.review.body.match(/\[[^\][]*]$/)[0].match(/\[(.*?)\]/)[1] ==
        selectedFilter
      );
    }
  });
...
filterOptions.forEach(filter => {
  filter.addEventListener('click', filterReviews);
});
...
function reviewArrayToHtml(arr) {
  const stampedReviewsHTML = arr.map(review => {
    ...
    const filter = body.match(/\[[^\][]*]$/)
      ? body.match(/\[[^\][]*]$/)[0].match(/\[(.*?)\]/)[1]
      : 'all';
    const sliderHTML = `<div class="swiper-slide" data-filter="${handleize(
      filter
    )}">
