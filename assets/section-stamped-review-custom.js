    ...
    filterOptions.forEach(filter => {
        filter.addEventListener('click', (e)=>{
            ...
            const review_mobile_selected_filter = document.querySelector('.mobile_review_filter_option')
            review_mobile_selected_filter.innerText = selectedFilter;
    ...
    var requestOptions = {
        ...
        function postReview(body){
            ...
            formdata.append("productId", "7216209494209");
    ...
    function mobileReviewHtmlGenaretor(arr){
        const mobile_pagination_container = document.querySelector('.mobile_review_pagination')
        mobile_pagination_container.innerHTML = `<div class="pagination_box"> ${paginationGenerator((Math.ceil(arr.length/3)))} </div>`;
        ...
        pagination_list.forEach(ele=>{
            ele.addEventListener("click",()=>{
                ...
                if(ele.getAttribute('pagination-page') == "next"){
                    page++;
                    const currentPagination = document.querySelector(`[pagination-page="${page}"]`);
    ...
    function mobile_review_html(arr){
        ...
        const stampedReviewsHTMLMobile = slicedArray.map(review=>{
            ...
            const review_html = `<div class="mobile_responsive_review_box">
                ...
                <div class="mobile_review_body">
                    <p class="gush-font-p2">${ body }</p>
                </div>
                <p class="gush-font-p2 author">${ author }</p>
            </div>`;
            return review_html;
        })
        const mobile_review_container = document.querySelector('.mobile-reviews-container');
        ...
        function paginationGenerator(pCount){
           let paginationList = '';
            ...
            for(let i=1;i<=pCount+1;i++){
                if(i<=pCount){
                    paginationList += `<li class="gush-font-teritary ${ i==1 && "pagination-active" }" pagination-page="${i}">${i}</li>`
                }else{
        ...
        const mobile_leave_review_btn = document.querySelector(".mobile_review_btn_show")
