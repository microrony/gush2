...
(function (modules, entry, mainEntry, parcelRequireName, globalName) {
  ...
  */ var OVERLAY_ID = "__parcel__error__overlay__";
  ...
  // menu button
  DOM.menuCtrl = document.querySelector('header-drawer summary[role="button"]');
  // menu container
  DOM.menuWrap = document.querySelector("#menu-drawer");
  // text content elements
  DOM.textContent = {
      heading1: document.querySelector("#MainContent h1"),
      heading2: document.querySelector("#MainContent h2"),
      heading3: document.querySelector("#MainContent h3"),
      heading4: document.querySelector("#MainContent h4"),
      heading5: document.querySelector("#MainContent h5"),
      heading6: document.querySelector("#MainContent h6"),
      button: document.querySelector("#MainContent .button"),
      paragraph: document.querySelector("#MainContent p"),
      list: document.querySelector("#MainContent ul"),
      bannerText: document.querySelector("#MainContent .banner_text")
  };
  Object.keys(DOM.textContent).forEach((key)=>{
      if (!DOM.textContent[key]) delete DOM.textContent[key];
  });
  // page state
  let state = 0;
  const textLinesRevealKeys = [
      "button",
      "paragraph",
      "list",
      "bannerText"
  ...
  const theTextLinesRevealElementsArray = Object.keys(DOM.textContent).filter((key)=>{
      return textLinesRevealKeys.includes(key);
  }).map((key)=>{
      return DOM.textContent[key];
  });
  let textLinesReveal = null;
  // Image reveal obj
  // const imgReveal = new ImgReveal(DOM.img);
  const textRevealKeys = [
      "heading1",
      "heading2",
      "heading3",
      "heading4",
  ...
  const theTextRevealElementsArray = Object.keys(DOM.textContent).filter((key)=>{
      return textRevealKeys.includes(key);
  }).map((key)=>{
      return DOM.textContent[key];
  });
  //[DOM.textContent.heading1, DOM.textContent.heading2, DOM.textContent.heading3, DOM.textContent.heading4, DOM.textContent.heading5, DOM.textContent.heading6, DOM.menuCtrl];
  let textReveal = null;
  // Menu
  // const menu = new Menu(DOM.menuWrap);
  // show the initial texts and images
  const showContent = ()=>{
      console.log("about to reveal text");
      // textReveal.in();
      // show texts (slide in)
      console.log("about to reveal text lines");
      textLinesReveal.in();
  // show image
  // imgReveal.in();
  // also show frame
  ...
  // });
  // Preload images
  // preloadImages('body img').then(() => {
  //     // Remove loader (loading class)
  //     /*
  //     add .oh and .oh__inner classes
  //      */
  // });
  (function() {
      console.log("theTextRevealElementsArray", theTextRevealElementsArray);
      theTextRevealElementsArray && theTextRevealElementsArray.length && theTextRevealElementsArray.forEach((ele)=>{
          console.log(ele);
          if (ele) {
              ele.classList.add("oh");
              const firstChild = ele.querySelector(":first-child");
              if (firstChild) firstChild.classList.add("oh__inner");
          }
      });
      console.log("theTextLinesRevealElementsArray theTextLinesRevealElementsArray", theTextLinesRevealElementsArray);
      textLinesReveal = new (0, _textLinesReveal.TextLinesReveal)(theTextLinesRevealElementsArray);
      console.log("theTextRevealElementsArray theTextRevealElementsArray", theTextRevealElementsArray);
      textReveal = new (0, _textReveal.TextReveal)(theTextRevealElementsArray);
      document.body.classList.remove("loading");
      // show content
      showContent();
  })();
  
  },{"./utils":"72Dku","./textReveal":"cUOGZ","./textLinesReveal":"2JTlA","gsap":"fPSuC"}],"72Dku":[function(require,module,exports) {
  ...
  const wrapLines = (elems, wrapType, wrapClass)=>{
      elems && elems.length && elems.forEach((char)=>{
  ...
  },{}],"cUOGZ":[function(require,module,exports) {
  var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
  parcelHelpers.defineInteropFlag(exports);
  ...
  class TextReveal {
      constructor(el){
          const innerElements = Array.isArray(el) ? el.filter((outer)=>{
              return !!outer;
          }) : null;
          console.log("innerElements", innerElements);
          this.DOM = {
              outer: el,
              inner: innerElements ? innerElements.map((outer)=>{
  ...
  */ /* eslint-disable */ var _gsapCoreJs = require("./gsap-core.js");
  ...
  },{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["aRq6p","ebWYT"], "ebWYT", "parcelRequire7b68")
